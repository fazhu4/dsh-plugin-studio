      ...
      {items.map(item => {
        ...
        return (
          <CardShell
            ...
            actions={(
              <>
                <span className="dsh_ps_badge">⭐ {item.stars}</span>
                <button
                  type="button"
  ...
  return (
    <div className="dsh_ps_section">
      <div className="dsh_ps_toolbar">
        ...
        <button type="button" className="dsh_ps_btn dsh_ps_btnPrimary" onClick={() => { void runSearch() }}>
          {t('search')}
        </button>
        {searching ? (
          <button type="button" className="dsh_ps_btn" onClick={backToBoard}>{t('backToBoard')}</button>
        ) : null}
      </div>
      <div className="dsh_ps_toolbar">
        <button
      ...
      {notice !== null ? <div className="dsh_ps_notice" data-error={notice.error ? 'true' : undefined}>{notice.text}</div> : null}
      {state.status === 'loading' ? <div className="dsh_ps_status">{t('marketSearching')}</div> : null}
      {state.status === 'error' ? (
        <div className="dsh_ps_status">
          {t('marketError')}
          <button type="button" className="dsh_ps_btn" onClick={() => { void loadBoard(boardTab) }}>{t('retry')}</button>
        </div>
      ) : null}
      {state.status === 'done' ? (
        state.items.length === 0
          ? <div className="dsh_ps_empty">{t('marketEmpty')}</div>
