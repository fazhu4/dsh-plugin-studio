...
async function writeGroups(file, entries) {
  const store = { version: 1, entries: Object.fromEntries(entries) };
...
async function buildList(ctx, paths) {
  ...
  const customGroups = [...customMap.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([name2, members]) => ({ name: name2, entries: members }));
  return {
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        await withLock(async () => {
          const overrides = await readGroups(paths.groupsFile);
          let changed = false;
          for (const [moduleName, name2] of [...overrides]) {
            if (name2 === groupName) {
              overrides.delete(moduleName);
              changed = true;
            }
